...
const [bs,core,mfmText,dcEntries]=await Promise.all([getJson(BS_URL),getJson(CORE_URL),getText(MFM_URL),Promise.all(DC_FILES.map(async name=>[name,await getJson(`${DC_BASE}/${name}.json`)]))]);
...
for(const entry of (bs.catalogue?.sharedSelectionEntries||[])){
  ...
  profiles.push({id:unitRaw.id,unitId:id,characteristics:unitRaw.characteristics});weapons.push(...all.filter(x=>x.type==='Ranged Weapons'||x.type==='Melee Weapons').map(x=>({...x,unitId:id})));abilities.push(...all.filter(x=>x.type==='Abilities').map(x=>({...x,unitId:id})));keywords.push(...categories.map(name=>({id:`necrons:keyword:${key(name)}:${key(entry.name)}`,keywordId:`necrons:keyword:${key(name)}`,name,unitId:id})));
...
const community40kdc=[{id:'necrons:community:40kdc',edition:'11th',faction:'necrons',upstream:DC_BASE,data:dc}];
const packages={units,profiles,weapons,abilities,keywords,detachments:detachmentRows,enhancements,stratagems,points,leaders,'core-rules':coreRules,'community-40kdc':community40kdc,source:[{id:'necrons:source:current',edition:'11th',faction:'necrons',bsdata:BS_URL,coreRules:CORE_URL,mfm:MFM_URL,community40kdc:DC_BASE,precedence:['BSData/wh40k-11e roster + core rules','BSData/wh40k-11e-mfm points','wn-mitch/40kdc-data supplemental rules/validation'],generatedAt:new Date().toISOString()}]};
