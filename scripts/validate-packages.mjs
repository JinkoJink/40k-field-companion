...
for(const [name,info] of Object.entries(packages)){
  ...
  if(![1,2].includes(payload.schemaVersion)||payload.package!==name||!Array.isArray(payload.records))throw new Error(`${name}: malformed package`);
  if(payload.edition && payload.edition!=='11th')throw new Error(`${name}: non-11e package`);
  if(payload.faction && payload.faction!=='necrons')throw new Error(`${name}: non-Necron package`);
  const ids=new Set;
  for(const row of payload.records){if(!row.id||ids.has(row.id))throw new Error(`${name}: duplicate stable ID`);ids.add(row.id);}
...
const community=rows['community-40kdc']?.[0];
if(community){
  if(community.faction!=='necrons'||community.edition!=='11th')throw new Error('community-40kdc: invalid scope');
  for(const unit of community.data?.units||[])if(unit.faction_id!=='necrons'||unit.game_version?.edition!=='11th')throw new Error(`community-40kdc: leaked non-Necron/non-11e unit ${unit.id}`);
}
console.log(`Validated ${Object.keys(packages).length} packages and ${units.size} Necron units.`);
